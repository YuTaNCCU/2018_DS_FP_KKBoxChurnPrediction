...
#performance
pred_logit <- ifelse(predict.glm(m_logit, newdata=test, type = "response" )>0.5, 1, 0 )
 perf(pred_logit)
...
pred_bag_temp <- foreach(m=1:iterations,.combine=rbind) %do% {
                  ...
                  #predict
                  pred_vector <- ifelse(predict.glm(m_beg,newdata=test, type = "response" )>0.5, 1, 0 )
                  return( pred_vector )
              }
...
ramdom_value <- runif(n = 8000, min = 0, max = 7450)
pred_null_train  <- ifelse(ramdom_value>550, 0, 1)
m_beg_8000 <- glm(is_churn ~ pred_null_train  , data = train, family=binomial(link="logit"))
