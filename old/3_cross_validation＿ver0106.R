...
#######################
{
  ...
  ggplot(df, aes(fill=variable, y=value, x=model, label=round(value,3))) +
    geom_bar(position="dodge",stat="identity") +
    theme(legend.position = "right") +
    labs(title="performance",subtitle="Version 01/07") +
    geom_text(aes(group=variable),position=position_dodge(width=0.9),size=3,vjust=-0.5,hjust=0.5) 
}
